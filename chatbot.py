...
def read_excel_or_csv(file_path):
    df = pd.read_csv("student_depression_dataset.csv") if file_path.endswith('.csv') else pd.read_excel(file_path)
    content = []
    for _, row in df.iterrows():
        content.append("\t".join(map(str, row)))
    return "\n".join(content)

os.environ["OPENAIPI_KEY"] = constants.APIKEY
...
embedding_model = OpenAIEmbeddings()  # Initialize the embedding model
